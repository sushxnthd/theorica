...
- A frozen black-box axiom-to-coordinate study recovered the symbolic latent-coordinate structure of **5/5** canonical associative operations, rejected **3/3** invalid structural controls, and met the frozen gates on **50/50** unseen noisy hidden-coordinate worlds. Median hidden-coordinate derivative-shape error was **0.001584** and median unseen operation error **0.0003931**; the structural representation beat a direct degree-6 bivariate polynomial baseline on **46/50** paired worlds. The clean GitHub Actions workflow reproduced the result and uploaded the artifact.
...
- The axiom-to-coordinate result does **not** claim that additive-generator representation theorems are new, nor that THEORICA is the first system to use them. The potentially novel claim is the end-to-end black-box experimental chain from axiom diagnosis to representation selection to latent-coordinate recovery and symbolic compression; this remains a **novelty hypothesis**, not a certified field-first claim.
...
## Translation-base reconstruction — 25 September 2026

### Supported

- For a finite operation whose left translations lie faithfully in a
  permutation group generated by `r` queried translations and whose action
  has a base of size `b`, the complete table is reconstructible from at most
  `n(r+b)` oracle values before optional validation.
- A natural subclass containing `n!` operations gives an
  information-theoretic `Omega(n)` chosen-query lower bound, so this scaling
  is order-optimal when `r+b=O(1)`.
- On the external published BOC task family, the frozen corrected run made
  **1,100/1,100** correct accept/reject decisions across 100 repetitions of 11
  operations.
- It accepted and exactly reconstructed **500/500** compact translation-action
  cases and produced **0/600** unexpected acceptances on the remaining tasks.
- Mean full-table query fractions were **2.752%** (modular addition),
  **4.761%** (modular subtraction), **3.377%** (S5 composition),
  **4.616%** (S5 conjugation), and **6.501%** (S5 sandwich `aba`).
- The representation-level compressor exhaustively recovered short action laws
  `XY`, `XYC`, `XYx`, and `XYX` on the corresponding external
  operations.
- A hostile targeted literature search found classical prior art for every
  component (permutation bases, black-box group recovery, rack/quandle
  permutation representations, operation-table learning) but did not locate an
...
- A guarantee for arbitrary quasigroups or operations with non-bijective or
  non-faithful translations.
- Direct sample-efficiency superiority to passive BOC methods such as
  HyperCube-SE, because the observation models differ.
- Superiority to specialized black-box group algorithms when the group family
  is supplied in advance.
- Robustness to arbitrarily sparse adversarial corruption.
...
> **Translation-base reconstruction is a plausible first-priority field-level
> methodological contribution: a family-agnostic selected-query method that
> discovers compact permutation-action representations of unknown finite
> operations, reconstructs their complete tables with conditionally
> order-optimal linear query scaling, and automatically recovers the induced
> action law.**

Use "first-ever" only as a **priority hypothesis**, not as a settled historical
fact, until independent experts have attempted to falsify it.

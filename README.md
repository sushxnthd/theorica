...
## Translation-base reconstruction

THEORICA now has a family-agnostic finite-operation discovery branch that does
not begin from named algebraic families.

For an unknown binary operation `F:SxS->S`, it treats each left translation

```
L_x(y)=F(x,y)
```

as a candidate permutation action. The learner queries only a few complete
translations, generates their permutation group, computes a small
permutation-group base, and uses the images of that base as an experimental
signature to identify every remaining translation. Unresolved signatures
trigger acquisition of another full row.

### Conditional recovery theorem

If all left translations lie in a permutation group `G`, the map
`x -> L_x` is faithful, `G` is generated by `r` translations, and the
action has a base of size `b`, then the entire `n x n` operation table is
recoverable from at most
...
- **1,100/1,100** accept/reject decisions correct;
- **500/500** compact-action cases accepted;
- **500/500** accepted cases reconstructed exactly;
- **0/600** unexpected acceptances.

Mean fractions of the full table queried:
...
The same representation-level law search then recovered, exhaustively,
`XY`, `XYC`, `XYx`, and `XYX` for the relevant operations without
receiving group/quandle/subtraction/sandwich labels.

A hostile search across black-box groups, operation-table reconstruction,
permutation-group algorithms, quasigroups, racks/quandles, and algebraic
learning found the ingredients separately but no exact predecessor for the
full family-agnostic translation-generator + group-base-signature reconstruction
procedure or the same conditional `n(r+b)` guarantee.

This is currently described as a **plausible first-priority field-level
methodological contribution**, not certified first-publication priority.

See:

- `docs/TRANSLATION_BASE_RECONSTRUCTION.md`
- `results/TRANSLATION_ACTION_BREAKTHROUGH_REPORT.md`
- `scripts/external_translation_action_holdout.py`

...
def cases():
    out=[]
    for i in range(25):
        d,a,_=make_world(6000+i)
        _,_,m=make_world(6100+i)
        out += [
            ("additive_generator",d,a,61000+i),
            ("quasi_arithmetic_mean",d,m,62000+i),
            ("commutative_semilattice",d,max if i%2==0 else min,63000+i),
        ]
        dc,fc=control(6200+i); out.append(("unresolved",dc,fc,64000+i))
        dd,fd=decoy(6300+i); out.append(("unresolved",dd,fd,65000+i))
    return out
...
def run():
    ...
    rows=[]
    # Frozen untouched confirmation after all router thresholds and stress
    # probes were fixed on the development panel.
    all_cases=cases()
    ...
    Path("results/query_efficient_router_confirmatory.json").write_text(
        json.dumps({"summary":summary,"rows":rows},indent=2)+"\n",encoding="utf-8"
    )
    print(json.dumps(summary,indent=2))
    assert all(s["correct"]==125 for s in summaries)

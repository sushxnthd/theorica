...
def summarize(records):
    ...
    f004 = np.array([r["f004_nrmse"] for r in ood], dtype=float)
    poly = np.array([r["poly12_nrmse"] for r in ood], dtype=float)
    paired = explicit - f004
    ...
    for family in sorted(set(r["family"] for r in records)):
        ...
        family_summary[family] = {
            ...
            "f004_median_nrmse": float(
                np.median([r["f004_nrmse"] for r in rows])
            ),
            ...
            "wins_vs_explicit": int(
                sum(r["f004_nrmse"] < r["explicit_nrmse"] for r in rows)
            ),
            "median_reject_step": (
    ...
    ratio = explicit / np.maximum(f004, 1e-15)
    return {
        "ood_n": len(ood),
        "control_n": len(controls),
        "ood_explicit_median_nrmse": float(np.median(explicit)),
        "ood_f004_median_nrmse": float(np.median(f004)),
        "ood_poly12_median_nrmse": float(np.median(poly)),
        "ood_median_error_ratio_explicit_over_f004": float(np.median(ratio)),
        "ood_wins_vs_explicit": int(np.sum(f004 < explicit)),
        "ood_wins_vs_poly12": int(np.sum(f004 < poly)),
        ...
        "paired_wilcoxon_one_sided_p": float(
            wilcoxon(explicit, f004, alternative="greater").pvalue
        ),
        ...
        "control_f004_median_nrmse": float(
            np.median([r["f004_nrmse"] for r in controls])
        ),
...
def run():
    ...
    for family, base in (("polynomial", 51000), ("trig", 51100)):
        for offset in range(20):
            records.append(evaluate_one(family, base + offset))
    ...
    payload = {
        ...
        "preregistration": "docs/F004_CONFIRMATORY_REPLICATION_PREREG.md",
        "summary": summary,
        "records": records,
    }
    out = Path("results/f004_anytime_representation_falsification.json")
    out.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(json.dumps(summary, indent=2))

    assert summary["ood_wins_vs_explicit"] >= 65
    assert summary["ood_f004_median_nrmse"] <= 0.03
    assert summary["ood_median_error_ratio_explicit_over_f004"] >= 7.0
    assert summary["ood_rejections"] >= 55
    assert summary["control_switches"] <= 3
    lo, hi = summary["paired_bootstrap_95ci"]
    assert lo > 0.0
    assert summary["paired_wilcoxon_one_sided_p"] < 1e-8

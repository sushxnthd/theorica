...
OUTPUT = Path("results/translation_action_challenge_v1_2.json")
...
def group_summary(records, source=None):
    ...
    adaptive_calls = np.asarray(
        [r["adaptive"]["calls"] for r in paired], dtype=float
    )
    random_calls = np.asarray(
        [r["random"]["calls"] for r in paired], dtype=float
    )
    if len(paired) and np.any(adaptive_calls != random_calls):
    ...
    return {
        ...
        "adaptive_mean_calls_paired": (
            float(np.mean(adaptive_calls)) if len(paired) else None
        ),
        "random_mean_calls_paired": (
            float(np.mean(random_calls)) if len(paired) else None
        ),
        ...
        "paired_wilcoxon_p_adaptive_less": paired_wilcoxon_p,
    }
...
def run():
    ...
    payload = {
        "experiment": "Translation-Action Challenge v1.2",
        "date": "2026-09-25",
        "corpus": {
    ...
    # Frozen v1.2 gates: identical to v1.1.
